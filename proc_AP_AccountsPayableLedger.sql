...
BEGIN
    ...
    FROM btggasify_purchase_live.tbl_grn_header gh
    INNER JOIN (SELECT DISTINCT grnid, poid FROM btggasify_purchase_live.tbl_grn_detail) gd ON gd.grnid = gh.grnid
    INNER JOIN btggasify_purchase_live.tbl_purchaseorder_header po ON po.poid = gd.poid
    LEFT JOIN btggasify_purchase_live.tbl_IRNReceipt_detail idtl ON CAST(idtl.grn_id AS UNSIGNED) = gh.grnid AND idtl.poid = po.poid
    LEFT JOIN btggasify_finance_live.tbl_claimAndpayment_header clm ON clm.irnid = idtl.receiptnote_hdr_id
    WHERE po.isactive = 1
