...
async def check_gascodes():
    ...
    try:
        async with engine_live.connect() as conn:
            print("--- Checking master_gascode ---")
            res = await conn.execute(text("SELECT Id, GasName FROM master_gascode WHERE Id IN (475, 31, 25, 469)"))
