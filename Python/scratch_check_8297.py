...
async def check_8297():
    ...
    try:
        async with engine_live.connect() as conn:
            print("--- Checking 8297 Details ---")
            res = await conn.execute(text("SELECT * FROM tbl_salesinvoices_details WHERE salesinvoicesheaderid = 8297"))
