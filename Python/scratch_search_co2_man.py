...
async def search_co2_man():
    ...
    try:
        async with engine_live.connect() as conn:
            print("--- Searching for CO2 MANIFOLD ---")
            res = await conn.execute(text("SELECT Id, GasName FROM master_gascode WHERE GasName LIKE '%CARBON%DIOXIDE%MANIFOLD%'"))
