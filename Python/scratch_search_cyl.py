...
async def search_cyl():
    ...
    try:
        async with engine_live.connect() as conn:
            print("--- Searching for OXYGEN CYLINDER 7.2 in master_gascode ---")
            res = await conn.execute(text("SELECT Id, GasName FROM master_gascode WHERE GasName LIKE '%OXYGEN%CYLINDER%7.2%'"))
