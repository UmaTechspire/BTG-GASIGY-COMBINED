...
async def search_72k():
    ...
    try:
        async with engine_live.connect() as conn:
            ...
            res = await conn.execute(text("SELECT id, salesinvoicesheaderid, gascodeid FROM tbl_salesinvoices_details WHERE UnitPrice = 72000"))
