...
async def check_details():
    ...
    try:
        async with engine_live.connect() as conn:
            print("--- Checking Details for Invoice 88932 Headers (2117 and 4362) ---")
            res = await conn.execute(text("SELECT salesinvoicesheaderid, COUNT(*) FROM tbl_salesinvoices_details WHERE salesinvoicesheaderid IN (2117, 4362) GROUP BY salesinvoicesheaderid"))
            print(f"Details count: {res.fetchall()}")
