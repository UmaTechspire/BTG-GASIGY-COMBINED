...
async def search_gases_3():
    url_live = f"mysql+aiomysql://{DB_USER}:{DB_PASS_QUOTED}@{DB_HOST}:{DB_PORT}/btggasify_live"
    engine_live = create_async_engine(url_live)
    
    queries = [
        "NITROGEN GAS CUSTOMER 7.2",
        "NITROGEN GAS CYLINDER 6.4",
        "NITROGEN GAS CYLINDER 7.2",
        "PURE NITROGEN%CUSTOMER%6.4",
        "PURE NITROGEN%CUSTOMER%1.5"
    ]
