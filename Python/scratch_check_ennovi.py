...
async def check_ennovi():
    ...
    try:
        async with engine_live.connect() as conn:
            print("--- Searching for ENNOVI ---")
            res = await conn.execute(text("SELECT Id, CustomerName FROM master_customer WHERE CustomerName LIKE '%ENNOVI%'"))
