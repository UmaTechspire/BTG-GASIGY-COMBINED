...
async def search_manpower():
    ...
    try:
        async with engine_live.connect() as conn:
            print("--- Searching for MANPOWER ---")
            res = await conn.execute(text("SELECT Id, GasName FROM master_gascode WHERE GasName = 'MANPOWER' OR GasName LIKE '%MANPOWER%'"))
