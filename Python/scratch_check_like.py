...
async def check_like():
    ...
    try:
        async with engine_live.connect() as conn:
            print("--- Checking 90470 LIKE ---")
            res = await conn.execute(text("SELECT id, salesinvoicenbr FROM tbl_salesinvoices_header WHERE salesinvoicenbr LIKE '%90470%'"))
            for row in res.fetchall():
                print(row)
                
            print("--- Checking 90523 LIKE ---")
            res = await conn.execute(text("SELECT id, salesinvoicenbr FROM tbl_salesinvoices_header WHERE salesinvoicenbr LIKE '%90523%'"))
            for row in res.fetchall():
                print(row)
