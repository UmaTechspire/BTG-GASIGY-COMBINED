...
async def search_oxygen():
    ...
    try:
        async with engine_live.connect() as conn:
            print("--- Searching for OXYGEN in master_gascode ---")
            res = await conn.execute(text("SELECT Id, GasName FROM master_gascode WHERE GasName LIKE '%OXYGEN%'"))
