...
async def search_pallet():
    ...
    try:
        async with engine_live.connect() as conn:
            print("--- Searching for OXYGEN PALLET in master_gascode ---")
            res = await conn.execute(text("SELECT Id, GasName FROM master_gascode WHERE GasName LIKE '%OXYGEN PALLET%'"))
