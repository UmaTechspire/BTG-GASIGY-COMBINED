...
async def search_gases_4():
    url_live = f"mysql+aiomysql://{DB_USER}:{DB_PASS_QUOTED}@{DB_HOST}:{DB_PORT}/btggasify_live"
    engine_live = create_async_engine(url_live)
    
    queries = [
        "PURE NITROGEN GAS CYLINDER 6.4",
        "PURE NITROGEN GAS CYLINDER 7.2",
        "MANPOWER",
        "NITROUS OXIDE%CUSTOMER%6.4",
        "NITROUS OXIDE%CYLINDER%6.4",
        "RENTAL CHARGE FOR ACETYLENE PALLET%6",
        "RENTAL CHARGE FOR ALL PALLET EXCEPT ACETYLENE",
        "RENTAL CHARGE FOR ALL CYLINDER",
        "RENTAL SPECIAL RACK%16",
        "RENTAL CHARGE FOR PALLET%4"
    ]
