...
async def check_h_ids():
    url_live = f"mysql+aiomysql://{DB_USER}:{DB_PASS_QUOTED}@{DB_HOST}:{DB_PORT}/btggasify_live"
    engine_live = create_async_engine(url_live)
    
    ids = [9179, 9261, 9750, 9766, 9903]
    
    try:
        async with engine_live.connect() as conn:
            print("--- Checking Header IDs ---")
            res = await conn.execute(text("SELECT id, salesinvoicenbr FROM tbl_salesinvoices_header WHERE id IN :ids"), {"ids": tuple(ids)})
