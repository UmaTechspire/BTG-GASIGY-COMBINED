...
async def check_ar_89713():
    ...
    try:
        async with engine_finance.connect() as conn:
            print("--- AR for 89713 ---")
            res = await conn.execute(text("SELECT inv_amount, balance_amount FROM tbl_accounts_receivable WHERE invoice_no = '89713'"))
            print(res.fetchone())
