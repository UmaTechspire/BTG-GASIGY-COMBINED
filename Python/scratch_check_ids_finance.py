...
async def check_ids_finance():
    ...
    try:
        async with engine_finance.connect() as conn:
            print("--- Checking Gas IDs in Finance ---")
            res = await conn.execute(text("SELECT Id, GasName FROM master_gascode WHERE Id IN (24, 78, 79, 177, 320)"))
