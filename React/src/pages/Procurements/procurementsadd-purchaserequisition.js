...
const AddPurchaseRequisition = () => {
    ...
    const refreshAttachmentList = async () => {
        if (!purchase_req_id) return;
        try {
            const data = await GetByIdPurchaseRequisition(purchase_req_id, branchId, orgId);
            if (data?.status && data.data) {
    ...
    const handleExistingFileDelete = async (rowData) => {
        try {
            const result = await Swal.fire({
                title: 'Are you sure?',
            ...
            if (result.isConfirmed) {
                const res = await DeletePrAttachment(rowData.prattachid);
                if (res.status) {
                    Swal.fire(
                        'Deleted!',
                    ...
                    setAttachments(prev => prev.filter(a => a.prattachid !== rowData.prattachid));
    ...
    const actionTemplate = (rowData, { rowIndex }) => {
        ...
        return (
            <span
                onClick={() => handleExistingFileDelete(rowData)}
                                                                        ...
                                                                        >
                                                                            {discussionHistory
                                                                                ...
                                                                                .map((msg, index, sortedArr) => {
                                                                                    ...
                                                                                    // Diff Logic - DISABLED to prevent hiding messages if backend returns individual rows
                                                                                    // if (index > 0) {
                                                                                    ...
                                                                                    // Extract sender from [User at Date]: or [User]:
                                                                                    ...
                                                                                    if (!cleanMessage) cleanMessage = msg.pr_comment; // Fallback if parsing resulted in empty
